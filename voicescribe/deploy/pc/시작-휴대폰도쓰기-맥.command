...
if [[ ! -x ".venv/bin/python" ]]; then
  ...
  if ! "$UV" venv .venv --python 3.11 || ! "$UV" pip install --python .venv/bin/python -e ".[fast,web,lan]"; then
    echo
...
# 3) 실행
echo "  [3/3] 시작합니다. 아래 주소나 QR 로 휴대폰에서 접속하세요."
echo
echo "  ------------------------------------------------"
echo "   끝내려면 이 창을 닫거나 Control+C 를 누르세요."
echo "  ------------------------------------------------"
echo
.venv/bin/python -m voicescribe.cli web --lan --https --no-browser
